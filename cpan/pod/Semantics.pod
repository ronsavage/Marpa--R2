...
And this Perl:
    ...
    my($recce) = Marpa::R2::Scanless::G -> new
    ({
        bless_package => 'Some::Package',
        source        => \$bnf,
    });

Then the effect of the C<object> reserved action name C<< action => ::first >>
combined with C<< bless => hash >> is to force the result of that rule into the
package name C<Some::Package::hash> when the rule is triggered. Hence we can do:
	...
	if ($type eq 'Some::Package::hash')
	{
		# Process $value_ref.
	}

C<bless_package> should not be confused with the L<SLIF recognizer's
C<semantics_package> named argument|Marpa::R2::Scanless::R/"semantics_package">
that provides the namespace in which action function names are searched for.
See below for details.

It is not normal practise to use both C<bless_package> and C<semantics_package>
in the same grammar.
...
Then the effect of the C<object> action verb C<< action => do_empty_object >>
combined with the C<semantics_package>, is to call the function
C<MarpaX::Demo::JSONParser::Actions::do_empty_object> when the rule is triggered.

C<semantics_package> should not be confused with the L<SLIF grammar's
C<bless_package> named argument|Marpa::R2::Scanless::G/"bless_package">
that names the class into which rule results are blessed.
...
L<quantified
rules|Marpa::R2::Scanless::DSL/"Quanitified rules">.
